...
MODELS = Literal[*available_models]
...
def load_model(model_name: MODELS):
    ...
    if model_name.lower() not in available_models:
        raise FileNotFoundError(f"The model {model_name} is not available chose from {available_models}")

    package_path: pathlib.Path = files("aimnet2")
